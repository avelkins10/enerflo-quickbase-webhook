...
const findFileUrl = (files, source) => {
  ...
  return file ? (file.url || file.externalUrl || '') : '';
};
...
function transformWebhookToQuickBase(webhook) {
  ...
  // Complete field mapping - ALL 150+ fields with accurate paths
  const recordData = {
    // Basic Deal Info
    ...
    94: { value: proposal?.pricingOutputs?.rebates?.[0]?.pricingOption?.inputs?.itcPercent ? Math.round(proposal.pricingOutputs.rebates[0].pricingOption.inputs.itcPercent * 100) : 30 }, // Federal ITC Percent
    38: { value: (proposal?.pricingOutputs?.downPayment || 0) * (proposal?.pricingOutputs?.grossCost || 0) }, // Down Payment Amount
    99: { value: Math.round((proposal?.pricingOutputs?.downPayment || 0) * 100) }, // Down Payment Percent
    ...
    98: { value: Math.round((proposal?.pricingOutputs?.dealerFeePercent || 0) * 100) }, // Dealer Fee Percent
    ...
    103: { value: Math.round((proposal?.pricingOutputs?.taxRate || 0) * 100) }, // Tax Rate
    ...
    54: { value: Math.round((proposal?.design?.offset || 0) * 100) }, // System Offset Percent
...
async function upsertRecord(dealId, recordData) {
  try {
    ...
    // Check if record was actually created or updated
    let recordId = null;
    let action = 'unknown';
